...
# $r13, $r14, $rbp, $rsp: address of $1
# $rbp: address of val[i-1]
# $r13 (index [i]): init = $rsp, limit = 
# $r12d (index [j]): init = 0, limit = 6
0x40110b <+23>:    mov    %rsp,%r14
0x40110e <+26>:    mov    $0x0,%r12d

 # If vals[i] > 6, then explode
0x401114 <+32>:    mov    %r13,%rbp               # $rbp move to next value
0x401117 <+35>:    mov    0x0(%r13),%eax
0x40111b <+39>:    sub    $0x1,%eax               # $eax = vals[i] - 1
0x40111e <+42>:    cmp    $0x5,%eax
0x401121 <+45>:    jbe    0x401128 <phase_6+52>
0x401123 <+47>:    call   0x40143a <explode_bomb>

# If vals[j] == vals[j-1], then explode
0x401128 <+52>:    add    $0x1,%r12d              # j++
0x40112c <+56>:    cmp    $0x6,%r12d
0x401130 <+60>:    je     0x401153 <phase_6+95>   # Check j range
